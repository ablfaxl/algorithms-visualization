...
export default function GraphVisualizer() {
  ...
  const bfs = useCallback(
    (nodes: Node[], edges: Edge[], start: number, end: number): GraphStep[] => {
      ...
      nodesClone.forEach((node) => {
        node.state =
          node.id === start ? "start" : node.id === end ? "end" : "default";
      });
      steps.push({
        ...
        currentNode: start,
        queue: [...queue],
        message: `شروع BFS از گره ${start} برای یافتن گره ${end}`,
        phase: "start",
      });
      ...
      while (queue.length > 0 && !found) {
        ...
        steps.push({
          ...
          currentNode: current,
          queue: [...queue],
          message: `در حال بررسی گره ${current}`,
          phase: "visiting",
        });
        ...
        for (const neighbor of adjList[current]) {
          if (!visited.has(neighbor)) {
            ...
            steps.push({
              ...
              currentNode: current,
              queue: [...queue],
              message: `گره ${neighbor} به صف اضافه شد`,
              phase: "visited",
            });
            ...
            if (neighbor === end) {
              ...
              steps.push({
                ...
                currentNode: null,
                queue: [...queue],
                message: `مسیر از ${start} به ${end} پیدا شد: ${path.join(
                  " → "
                )}`,
                phase: "path",
              });
  ...
  const startBFS = useCallback(() => {
    if (steps.length === 0) {
      const bfsSteps = bfs(nodes, edges, startNode, endNode);
      setSteps(bfsSteps);
    }
    setIsPlaying(true);
  }, [nodes, edges, startNode, endNode, bfs, steps.length]);
  ...
  const pauseBFS = () => setIsPlaying(false);
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle className="text-3xl font-bold text-center text-gray-800">
              نمایش بصری الگوریتم جستجوی سطحی (BFS)
            </CardTitle>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-4 justify-center items-center mb-6">
              ...
              <Button
                onClick={isPlaying ? pauseBFS : startBFS}
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-xl">مراحل الگوریتم BFS</CardTitle>
                </CardHeader>
                <CardContent className="text-right space-y-4">
                  <div className="border-r-4 border-blue-500 pr-4">
                    ...
                    <p className="text-sm text-gray-600">
                      گره شروع به صف اضافه می‌شود و به عنوان بازدیدشده
                      علامت‌گذاری می‌شود.
                    </p>
                  </div>
                  <div className="border-r-4 border-yellow-500 pr-4">
                    ...
                    <p className="text-sm text-gray-600">
                      گره‌ها به ترتیب از صف خارج شده و همسایه‌های بازدیدنشده
                      آن‌ها بررسی می‌شوند.
                    </p>
                  </div>
                  <div className="border-r-4 border-purple-500 pr-4">
                    ...
                    <p className="text-sm text-gray-600">
                      همسایه‌های بازدیدنشده به صف اضافه می‌شوند.
                    </p>
                  </div>
                  <div className="border-r-4 border-green-500 pr-4">
                    ...
                    <p className="text-sm text-gray-600">
                      فرآیند تا یافتن گره پایان یا خالی شدن صف ادامه می‌یابد.
                    </p>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="text-right space-y-3">
                  ...
                  <div className="mt-4 p-3 bg-cyan-50 rounded-lg">
                    <p className="text-sm text-cyan-800">
                      <strong>نکته:</strong> BFS کوتاه‌ترین مسیر را در گراف‌های
                      بدون وزن تضمین می‌کند.
                    </p>
                  </div>
                </CardContent>
              </Card>
